					...
					<>
						...
						{messages.length > maxVisibleMessages && (
							<Box paddingX={1} paddingY={0}>
								<Text
									color={isScrollFocused ? 'yellow' : 'gray'}
									dimColor={!isScrollFocused}
								>
									{isScrollFocused && '[SCROLL MODE] '}
									{scrollOffset > 0 &&
										`↑ Scroll up (${scrollOffset} messages above)`}
									{scrollOffset > 0 &&
										scrollOffset < messages.length - maxVisibleMessages &&
										' | '}
									{scrollOffset < messages.length - maxVisibleMessages &&
										`↓ Scroll down (${
											messages.length - scrollOffset - maxVisibleMessages
										} messages below)`}
								</Text>
							</Box>
						)}
