		...
		(input, key) => {
			...
			if (key.backspace || key.delete) {
				...
				if (cursorX > 0) {
					...
					const newLine =
						currentLine.slice(0, cursorX - 1) + currentLine.slice(cursorX);
					const newLines = [...lines];
			...
			if (input && !key.ctrl && !key.meta) {
				const currentLine = lines[cursorY];
				const newLine =
					currentLine.slice(0, cursorX) + input + currentLine.slice(cursorX);
				const newLines = [...lines];
				newLines[cursorY] = newLine;
				setLines(newLines);
				setCursorX(cursorX + 1);
				...
				<Box key={index} flexDirection="row">
					<Text>
						{index === cursorY && index < maxHeight && focus ? (
							...
							line ||
							(index === 0 && !focus && <Text dimColor>{placeholder}</Text>)
						)}
					</Text>
				</Box>
			))}
